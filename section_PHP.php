...
<!--//  HY 2018/12/11 -->
#闭包+递归，实现从数组的根节点（默认为空,可指定任意名称）开始，得到该数组从根节点开始到最后一层节点的所有路径的数组。
...
#实现闭包的递归调用
$pathFind=function($arr,$nodeName='')use(&$pathArr,&$pathFind){
    $subKeys=[];
    $subVals=[];
	...
	#判断当前节点名是否为最后一层的节点名。不是最后一层的节点名，返回本层节点名与下一层节点名组合后的数组，递归操作
    if(is_string($subKeys[0]) && is_array($subVals[0])){
        //先删除当前节点名。
        array_splice($pathArr,array_search($nodeName,$pathArr),1);
        foreach($subKeys as $v){
            if($v!='admin' && !empty($nodeName)){
              $adStr=$nodeName.'-'.$v;
            }else if($v!='admin' && empty($nodeName)){
              $adStr=$v;
            }
            array_push($pathArr,$adStr);
            //递归调用本闭包
			$pathFind($arr[$v],$adStr);
        }
    }
};
#调用闭包函数，设定$pathArr的值。
$pathFind($test);
#得到结果：$pathArr=['ass','att','iss-pat','iss-pro','iss-the','pro','the']

$dbAuth=["ass"=>["read"=>1],"att"=>["upload"=>1],"iss"=>["pat"=>["edit"=>1],"pro"=>["edit"=>1],"the"=>["edit"=>1]],"pro"=>["edit"=>1,"audit"=>1],"the"=>["edit"=>1,"audit"=>1,"execute"=>1]];

$conAuth=["_ASS"=>["read"=>0,"write"=>0],"_ATT"=>["upload"=>0],"_ISS"=>["_PAT"=>["edit"=>1],"_PRO"=>["edit"=>1],"_THE"=>["edit"=>1]],"_PRO"=>["edit"=>1,"audit"=>1],"_THE"=>["edit"=>1,"audit"=>1,"execute"=>1],"_ADMIN"=>["enable"=>0]];

/**
     * 比较$dbAuth数组和$conAuth，补足$dbAuth中缺失的部分。
	 * $dbAuth数组和$conAuth
     * @param  Array $dbAuth，目标多维关联数组（json对象转换来的数组），地址引用。
     * @param  Array $conAuth，基准多维关联数组，地址引用。
